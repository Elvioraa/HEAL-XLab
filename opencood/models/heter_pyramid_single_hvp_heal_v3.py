"""HVP-HEAL v3 feature-main wrapper for HEAL single-modality pyramid.
...
class HeterPyramidSingleHvpHealV3(HeterPyramidSingle):
    """HEAL Stage2 single model with optional HVP-v3 evidence head."""

    HVP_V3_MODULE_PREFIXES = ("hvp_v3_evidence_head",)
    ...
    def __init__(self, args):
        ...
        if self._is_stage2_enabled():
            head_cfg = self.hvp_v3_cfg["evidence_head"]
            self.hvp_v3_evidence_head = HvpHealV3EvidenceHead(
            ...
            self._apply_stage2_train_mode()
            check_trainable_module(self)

    def forward(self, data_dict):
        if not self._is_stage2_enabled():
            return super().forward(data_dict)
        return self._forward_stage2_evidence(data_dict)

    def load_state_dict(self, state_dict, strict=True):
        """Load only compatible keys so Stage1/base checkpoints can seed Stage2."""
        ...
        for key, value in state_dict.items():
            ...
            if own_value is not None and getattr(value, "shape", None) == own_value.shape:
                ...
                skipped.append(key)
        self._hvp_v3_last_skipped_state_keys = skipped[:80]
        return super().load_state_dict(filtered, strict=False)

    def _forward_stage2_evidence(self, data_dict):
        ...
        self._append_hvp_v3_stage2_output(output_dict, feature, modality_name)
        return output_dict
    ...
    def _append_hvp_v3_stage2_output(self, output_dict, bev_feature, modality_name):
        head_output = self.hvp_v3_evidence_head(bev_feature)
        output_dict["hvp_v3"] = {
            "enabled": True,
            "stage": "stage2_evidence",
            "train_mode": self.hvp_v3_cfg["stage2"].get(
                "train_mode",
                "evidence_adaptation",
            ),
            "modality": modality_name,
            "use_hvp_v3_evidence": bool(
                self.hvp_v3_cfg["stage2"].get("use_hvp_v3_evidence", False)
            ),
            ...
            "evidence_descriptor": head_output["evidence_descriptor"],
            "evidence_loss_cfg": self.hvp_v3_cfg["evidence_loss"],
        }
        if "evidence_feature" in head_output:
            output_dict["hvp_v3"]["evidence_feature"] = head_output["evidence_feature"]
        return output_dict

    def _is_stage2_enabled(self):
        return (
            bool(self.hvp_v3_enabled)
            and self.hvp_v3_stage == "stage2_evidence"
            and bool(self.hvp_v3_cfg["stage2"].get("use_hvp_v3_evidence", False))
            and bool(self.hvp_v3_cfg["evidence_head"].get("enabled", False))
        )

    def _apply_stage2_train_mode(self):
        if not self._is_stage2_enabled():
            return
        for param in self.hvp_v3_evidence_head.parameters():
    ...
    def _normalize_hvp_v3_cfg(cls, cfg):
        ...
        stage2["train_mode"] = str(stage2.get("train_mode", "evidence_adaptation"))
        stage2["use_hvp_v3_evidence"] = bool(
            stage2.get("use_hvp_v3_evidence", False)
        )
        ...
        loss["mode"] = str(loss.get("mode", "stage2_evidence"))
